...
# --------------------------------------------------------------------
        ...
        for model, style, color in zip(models, linestyles, colors):
            ...
            #Cada vez que se realiza el ciclo se crea una trama, es decir un grupo de ordenadas x e y

        plt.legend(["d=%i" % m.order for m in models], loc="upper left")
        #Nos muestra el nombre de las lineas que vamos a graficar 

    plt.autoscale(tight=True)
    #Ajuste automático de escala para la visión
    plt.ylim(ymin=0)
    #Define el limite inferior de Y
    if ymax:
        plt.ylim(ymax=ymax)
    if xmin:
        plt.xlim(xmin=xmin)
    ...
    #Define las propiedas de la cuadrícula, true para que sea visible
    plt.savefig(fname)
...
#no entienod esta parte

# Crea y dibuja los modelos de datos
# -----------------------------------------------------------------
fp1, res1, rank1, sv1, rcond1 = np.polyfit(x, y, 1, full=True)
# Apropxima un conjunto de datos según minimos cuadrados
print("Parámetros del modelo fp1: %s" % fp1)
print("Error del modelo fp1:", res1)
f1 = sp.poly1d(fp1)
#encapsula operaciones "naturales" en polinomios para que dichas operaciones puedan tomar su forma habitual en el código
...
f2 = sp.poly1d(fp2)
#Encapsula operaciones naturales en polinomios
...
xb = x[int(inflexion):]
yb = y[int(inflexion):]
#Busca en los arrays el correspondiente ocn el punto de infleccion y lo guarda en nuevas variables 
...
for f in [f1, f2, f3, f10, f100]:
    ...
    #For para mostrar los posibles errores de los arrays 
...
#For para mostrar los errores de los datos pero después del punto de inflexión
